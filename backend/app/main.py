...
from .tinkoff_client import create_tinkoff_payment, get_tinkoff_payment_state, generate_token
...
async def tinkoff_webhook(request: Request):
    ...
    # Проверка подписи
    calc_token = generate_token(payload)
    ...
    try:
        ...
        if s in ("confirmed", "completed", "authorized", "success"):
            order.status = "paid"
        elif s in ("reversed", "refunded", "failed", "declined", "rejected", "canceled"):
